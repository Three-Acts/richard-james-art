 ...
 * The year navigator.
 *
 * Desktop: a vertical list pinned at the left edge — 2022 … 2025 plus
 * "View All". The active year (= the active project's year) is lit gold.
 * Mobile: the same set as a horizontal chip row.
 *
 * Clicking a year jumps to the FIRST project of that year (View All → first
 * project overall) via the supplied goToIndex.
 */
export default function YearFilter({
  ...
  const entries: { key: string; label: string; index: number }[] = [
    ...years.map((y) => ({ key: y, label: y, index: firstIndexByYear[y] ?? 0 })),
    { key: 'all', label: 'View All', index: 0 },
  ]
  ...
  return (
    <nav
      ...
      className="pointer-events-none absolute inset-y-0 left-0 z-40 hidden items-center md:flex"
      style={{ paddingLeft: 'var(--gutter)' }}
    >
      <ul className="pointer-events-auto flex flex-col gap-4">
        {entries.map((e) => {
          const isActive = e.key === activeYear
          const isAll = e.key === 'all'
          return (
            <li key={e.key} className={isAll ? 'mt-3' : undefined}>
              <button
                type="button"
                onClick={() => goToIndex(e.index)}
                aria-current={isActive ? 'true' : undefined}
